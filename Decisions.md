...
- **Why:** When a session is purged or database files are deleted, ChromaDB raises a generic `ValueError` when querying. Translating this to an explicit HTTP 404 lets the frontend know exactly when to prompt the user to re-upload their document.## 7. Streamlit Frontend Client (2026-08-12)

...
## 10. Query Expansion (2026-08-16)
- **What:** Implemented query expansion using Groq to generate 2 alternative legal/financial queries for every user question.
- **Why:** Standard user questions are often phrased in plain language (e.g. "closing early"), whereas contracts use precise terminology ("prepayment"). Generating synonyms and searching for multiple variants increases vector and sparse retrieval recall.

## 11. Multi-Container Dockerization (2026-08-16)
- **What:** Packaged the application using Docker, defining `app/Dockerfile`, `frontend/Dockerfile`, and `docker-compose.yml`.
- **Why:** Isolates dependencies, enables persistent volume mounting for the local database, and allows single-command startup. Optimizes the frontend container size by omitting heavy ML packages (e.g., torch) and installing only Streamlit/requests.
